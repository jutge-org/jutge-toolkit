...
export async function complete(jutge: JutgeApiClient, model: string, label: string, systemPrompt: string, userPrompt: string): Promise<string> {
...
export class ChatBot {
    ...
    private async interact(): Promise<string> {
        ...
        const { id } = await this.jutge.instructor.jutgeai.chat({ model: this.model, label: this.label, messages: this.messages, addUsage: false })
