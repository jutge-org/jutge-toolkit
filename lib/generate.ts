...
import type { Problem } from './new-problem'
import { settings } from './settings'
import tui from './tui'
import { projectDir, readText, readTextInDir, stripLaTeX, writeTextInDir, writeYamlInDir } from './utils'
import { exists } from 'fs/promises'

export async function addStatementTranslation(model: string, problem: Problem, language: string) {
...
export async function addAlternativeSolution(model: string, problem: Problem, extension: string) {
...
export async function addMainFile(model: string, problem: Problem, proglang: string) {
    if (!problem.goldenSolution) {
        throw new Error('No golden solution defined')
    }
