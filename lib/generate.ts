...
The texts that the program must read and write should not be translated.
    `
        ...
        const answer = cleanMarkdownCodeString(await complete(jutge, model, "add-statement-translation", prompt, statememt))
...
${proglangPrompt}
    ...
    const answer = cleanMarkdownCodeString(await complete(jutge, model, "add-alternative-solution", prompt, goldenSource))
...
export async function generateTestCasesGenerator(
    ...
    await tui.section(`Generating test cases generator ${outputPath}`, async () => {
        ...
        const answer = cleanMarkdownCodeString(await complete(jutge, model, "generate-test-cases-generator", prompt, prompt))
