...
import { cleanMarkdownCodeString, complete } from './aiclient'
...
export async function generateStatementFromSolution(
    ...
    const statementCoda = await readText(
        join(projectDir(), 'assets', 'prompts', 'io', 'examples', 'io-statement-coda.tex'),
    )
...
export async function addStatementTranslation(
    jutge: JutgeApiClient,
    model: string,
    problem: Problem,
    language: string,
...
The texts that the program must read and write should not be translated.
    `
        ...
        const answer = cleanMarkdownCodeString(
            await complete(jutge, model, 'add-statement-translation', prompt, statememt),
        )
...
export async function addAlternativeSolution(
    jutge: JutgeApiClient,
    model: string,
    problem: Problem,
    extension: string,
...
${proglangPrompt}
    ...
    const answer = cleanMarkdownCodeString(
        await complete(jutge, model, 'add-alternative-solution', prompt, goldenSource),
    )
...
${proglangPrompt}
    `
        ...
        const answer = cleanMarkdownCodeString(await complete(jutge, model, 'add-main-file', prompt, goldenSource))
...
export async function generateTestCasesGenerator(
    ...
    await tui.section(`Generating test cases generator ${outputPath}`, async () => {
        ...
        const answer = cleanMarkdownCodeString(
            await complete(jutge, model, 'generate-test-cases-generator', prompt, prompt),
        )
