...
export const submitCmd = new Command('submit')
    ...
    .argument('<sources...>', 'source files to submit (e.g. solution.cc slow.py buggy.py)')
    ...
    .action(async (sources, { directory, language, wait }) => {
        await submitInDirectory(directory, language, sources, !wait)
    })
